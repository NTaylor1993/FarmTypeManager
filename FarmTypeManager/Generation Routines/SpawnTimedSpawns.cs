                ...
                try
                {
                    ...
                    for (int x = timedSpawns.Count - 1; x >= 0; x--) //for each list of spawns (looping backward for removal purposes)
                    {
                        ...
                        for (int y = spawns.Count - 1; y >= 0; y--) //for each object to be spawned (looping backward for removal purposes)
                        {
                            ...
                            {
                                Utility.Monitor.VerboseLog($"Failed to spawn an object on a seemingly valid tile, e.g. due to low tile safety settings. Mod ID: {spawns[y].FarmData?.Pack?.Manifest.UniqueID ?? "N/A (local data folder or missing info)"}. Type: {spawns[y].SavedObject.Type}. Location: {spawns[y].SavedObject.Tile.X},{spawns[y].SavedObject.Tile.Y} ({location.Name}).");
                            }
                        }
                    ...
                    if (spawnedTotal > 0)
                        Utility.Monitor.Log($"Spawned {spawnedTotal} objects. Time: {time?.Time.ToString() ?? "(any)"}.", LogLevel.Trace);
                }
                catch (Exception ex)
                {
                    Utility.Monitor.Log($"An error occurred while spawning. Some objects or monsters will probably fail to spawn. Intended spawn time: {time?.Time.ToString() ?? "(any)"}. Full error message: \n{ex.ToString()}", LogLevel.Error);
                }
            }
        }
    }
